...
use crate::streaming::{
    ConnectionId, StreamContext, StreamManager, StreamMessage, StreamResult, StreamError,
    DataDirection, CloseReason, MetricsCollector, ErrorRecoveryCoordinator,
};
...
impl StreamManager for BidirectionalStreamManager {
    ...
    async fn create_bidirectional_stream(
        ...
        info!("Creating bidirectional stream for connection {}", connection_id);
        ...
        let client_to_agent_task = {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    match read_result {
                        Ok(0) => {
                            ...
                            let _ = agent_sender.send(StreamMessage::Close {
                                connection_id,
                        ...
                        Ok(bytes_read) => {
                            ...
                            if let Some(metrics) = &metrics_collector {
                                metrics.record_data_transfer(connection_id, bytes_read as u64, "sent").await;
                            }
                            ...
                            if let Err(e) = agent_sender.send(message).await {
                                let error = StreamError::channel_send(format!("Failed to send data to agent: {}", e));
                                ...
                                if let Some(recovery) = &recovery_coordinator {
                                    let error_context = error.with_connection_context(connection_id, "data_send");
                                    if let Err(recovery_err) = recovery.attempt_recovery(error_context, Some(connection_id)).await {
                                        error!("Recovery failed for connection {}: {}", connection_id, recovery_err);
                                    }
                                }
                                
                                error!("Failed to send data to agent for connection {}: {}", connection_id, e);
                                break;
                            }
                            
                            debug!("Forwarded {} bytes from client to agent for connection {}", bytes_read, connection_id);
                        }
                        Err(e) => {
                            let error = StreamError::stream_broken(connection_id, format!("Error reading from client: {}", e));
                            ...
                            // Attempt error recovery
                            if let Some(recovery) = &recovery_coordinator {
                                let error_context = error.with_connection_context(connection_id, "client_read");
                                if let Err(recovery_err) = recovery.attempt_recovery(error_context, Some(connection_id)).await {
                                    error!("Recovery failed for connection {}: {}", connection_id, recovery_err);
                                }
                            }
                ...
                debug!("Client-to-agent stream handler finished for connection {}", connection_id);
            })
        };
        ...
        info!("Bidirectional stream created successfully for connection {}", connection_id);
        Ok(())
    }
    ...
    async fn handle_agent_stream(
        ...
        debug!("Handling agent stream data for connection {}: {} bytes", connection_id, agent_data.len());

        let streams = self.streams.read().await;
        let stream_context = streams.get(&connection_id)
            .ok_or_else(|| {
        ...
        match write_result {
            Ok(()) => {
                ...
                if let Some(metrics) = &self.metrics_collector {
                    metrics.record_data_transfer(connection_id, agent_data.len() as u64, "received").await;
                }
                
                debug!("Successfully forwarded {} bytes from agent to client for connection {}", agent_data.len(), connection_id);
                Ok(())
            }
            Err(e) => {
                let error = StreamError::stream_broken(connection_id, format!("Failed to write to client: {}", e));
                ...
                if let Some(recovery) = &self.recovery_coordinator {
                    let error_context = error.with_connection_context(connection_id, "agent_to_client_write");
                    let recovery_coordinator = Arc::clone(recovery);
                    tokio::spawn(async move {
                        if let Err(recovery_err) = recovery_coordinator.attempt_recovery(error_context, Some(connection_id)).await {
                            error!("Recovery failed for connection {}: {}", connection_id, recovery_err);
                        }
                    });
                }
                
                error!("Failed to write agent data to client for connection {}: {}", connection_id, e);
    ...
    async fn terminate_stream(&self, connection_id: ConnectionId) -> StreamResult<()> {
        ...
        if stream_context.is_none() {
            warn!("Attempted to terminate non-existent stream {}", connection_id);
            return Ok(());
        }
        ...
        self.agent_sender.send(close_message).await
            .map_err(|e| StreamError::channel_send(format!("Failed to send close message: {}", e)))?;
    ...
    async fn send_to_agent(&self, connection_id: ConnectionId, data: Bytes) -> StreamResult<()> {
        ...
        self.agent_sender.send(message).await
            .map_err(|e| StreamError::channel_send(format!("Failed to send data to agent: {}", e)))?;
...
mod tests {
    ...
    async fn tcp_pair() -> Option<(TcpStream, TcpStream)> {
        ...
        let client = TcpStream::connect(addr).await.expect("client connect failed");
    ...
    async fn test_agent_channel_communication() {
        ...
        // Send data to agent
        let result = manager.send_to_agent(connection_id, test_data.clone()).await;
        assert!(result.is_ok());
        ...
        match message {
            StreamMessage::Data { connection_id: recv_id, payload, direction } => {
    ...
    async fn test_create_bidirectional_stream() {
        ...
        let result = manager.create_bidirectional_stream(connection_id, server_stream).await;
        assert!(result.is_ok());
    ...
    async fn test_stream_lifecycle() {
        ...
        match message {
            StreamMessage::Close { connection_id: recv_id, reason } => {
                assert_eq!(recv_id, connection_id);
