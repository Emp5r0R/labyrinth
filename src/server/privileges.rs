...
impl PrivilegeManager {
    ...
    pub fn display_sudo_warning() {
        println!("{}", styling::format_warning_msg(
            styling::WARNING_INDICATOR,
            "Running without sudo privileges"
        ));
        println!("{}Some features may be limited:", styling::INDENT_LEVEL_1);
        println!("{}• Fullhouse mode (TUN interface creation)", styling::INDENT_LEVEL_2);
        println!("{}• Network interface manipulation", styling::INDENT_LEVEL_2);
        println!("{}• System-level routing changes", styling::INDENT_LEVEL_2);
        println!("{}Run with 'sudo' for full functionality", styling::INDENT_LEVEL_1);
        println!();
    }
